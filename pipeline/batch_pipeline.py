...
def run_one(i, total, pdf):
    ...
    try:
        ...
        return {"pdf_file": pdf.name, "status": "failed", "error": str(e), "report_dir": str(out), "appendix_found": "", "candidate_pages": "", "total_fields": 0, "extracted_fields": 0, "missing_fields": 0, "coverage_rate": 0, "raw_row_count": 0, "unknown_metrics": 0}


def write_summary(rows):
    path = OUTPUT_DIR / "batch_summary.csv"
    path.parent.mkdir(parents=True, exist_ok=True)
    fields = ["pdf_file", "status", "error", "report_dir", "appendix_found", "candidate_pages", "raw_row_count", "total_fields", "extracted_fields", "missing_fields", "coverage_rate", "unknown_metrics"]
...
def run_batch():
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        fs = [ex.submit(run_one, i, len(pdfs), p) for i, p in enumerate(pdfs, 1)]
